    ...
    watch(arrayValue, () => {
     ...
     async function sort() {
        if (selectedSort.value === "Insertion Sort") {
            ...
            await bubbleSort();
        } else if (selectedSort.value === "Selection Sort") {
            await selectionSort();
        }
        // console.log(columns);
    }
...
async function selectionSort()
{
     ...
     for (i = 0; i < columns.length-1; i++)
    {
        ...
        for (j = i + 1; j < columns.length; j++) {
            if (columns[j] < columns[min_idx])
                min_idx = j;
        }
        // Swap the found minimum element with the first element
        swap(columns,min_idx, i);
        await sleep();
        updateDOM.value+=1;
    }
}
        ...
        <div class="container">
            <o-dropdown :triggers="['hover']" v-model="selectedSort" aria-role="list">
                ...
                <div>
                    ...
                    <o-dropdown-item aria-role="listitem" value="Bubble Sort">Bubble Sort</o-dropdown-item>
                    <o-dropdown-item aria-role="listitem" value="Selection Sort">Selection Sort</o-dropdown-item>
                </div>
            </o-dropdown>
        </div> 
